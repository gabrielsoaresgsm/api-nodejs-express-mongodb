...
router.post('/', async (req, res) => {
    ...
    if(!firstName) {
        res.status(422).json({message: 'Campo obrigatorio 1'}) // Erro, caso os dados nao foram criados com sucesso
        return
    } else if (!lastName){
        res.status(422).json({message: 'Campo obrigatorio 2'})
        return
    } else if (!phone){
        res.status(422).json({message: 'Campo obrigatorio 3'})
        return
    } else if (!email){
        res.status(422).json({message: 'Campo obrigatorio 4'})
        return
    }
    ...
    try{
        ...
        res.status(201).json({message: 'Agendamento inserido com sucesso'}) // Dados criado com sucesso
